...
public:
    ...
    enum LineJoin
    {
        XXXJoinMiter = agg::miter_join,
        XXXJoinRound = agg::round_join,
        XXXJoinBevel = agg::bevel_join
    };

    enum LineCap
    {
        XXXCapButt   = agg::butt_cap,
        XXXCapSquare = agg::square_cap,
        XXXCapRound  = agg::round_cap
    };
	...
	bool hittestPath(int x, int y, bool noFill = false, DrawPathFlag flag = FillAndStroke);
    ...
    void dashAdd( double dash_len, double gap_len );
    void dashStart( double dash_start );
    double dashStart()          const { return m_dashStart; }
    void dashClear();
