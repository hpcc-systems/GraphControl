...
Agg2D::Agg2D() :
    ...
    m_lineCap(XXXCapRound),
    m_lineJoin(XXXJoinRound),
...
void Agg2D::dashAdd( double dash_len, double gap_len )
{
    m_convDash.add_dash( dash_len, gap_len );
    m_dashes.push_back( Dash( dash_len, gap_len ) );
}

void Agg2D::dashStart( double dash_start )
{
...
bool Agg2D::hittestText(double hit_x, double hit_y, double x, double y, const char* str, bool roundOff, double ddx, double ddy)
{
   ...
   agg::trans_affine  mtx;
    ...
    return hit_x >= start_x && hit_x < start_x+textWidth(str) && hit_y >= start_y - asc && hit_y < start_y;
}
...
bool Agg2D::hittestPath(int x, int y, bool noFill, DrawPathFlag flag)
{
    m_rasterizer.reset();
	if (!noFill)
		m_rasterizer.add_path(m_pathTransform);
    m_rasterizer.add_path(m_strokeTransform);
	return m_rasterizer.hit_test(x, y);
}
